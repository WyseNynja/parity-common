...
fn bytes_to_hex(c: &mut Criterion) {
	let parameters = vec![
		...
		serde_json::from_str::<Bytes>(&input::HEX_640_CHARS).unwrap(),
		serde_json::from_str::<Bytes>(&input::HEX_6400_CHARS).unwrap(),
		serde_json::from_str::<Bytes>(&input::HEX_64000_CHARS).unwrap(),
		serde_json::from_str::<Bytes>(&input::HEX_640000_CHARS).unwrap(),
	];
...
fn hex_to_bytes(c: &mut Criterion) {
	let parameters = vec![
		input::HEX_64_CHARS,
		input::HEX_640_CHARS,
		input::HEX_6400_CHARS,
		input::HEX_64000_CHARS,
		input::HEX_640000_CHARS,
	];
